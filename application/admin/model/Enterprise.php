...
{
     protected $pk = 'id';
     protected $table = 'demo_enterprise';
    ...
    public function get($paramArr = array())
    {
        try{
            $where = 'status = 1 ';
            if($paramArr['keyword']){    //搜索描述
                isset($paramArr['keyword']) && $where .= "and  name like ". "'%".$paramArr['keyword']."%'";
            }
            return Db::name('enterprise')->where($where)->order('id')->paginate(10)->toArray();
    ...
    {
        try{
            Db::name('enterprise')->where('id',$paramArr['id'])->delete();
    ...
    {
        try{
            $result = Db::name('enterprise')->where('id',$paramArr['id'])->data($paramArr)->update();
